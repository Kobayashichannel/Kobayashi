...
public class HouseService {
    ...
    public HouseService(int size) {
        //new houses
        houses = new House[size]; //当创建
        //为了配合测试列表信息,老韩这里初始化一个,指定数组大小
    ...
    public boolean del(int delId) {
        ...
        int index = -1;
        for (int i = 0; i < houseNums; i++) {
            if(delId == houses[i].getId()) { //删除房屋(id),在数组下标i的元素
                index = i; //就使用index记录i
            }
        }

        if(index == -1) {//说明delitId在数组中不存在
            return false;
        }
        //如果找到,这里需要小伙伴动脑筋,有点难
        //老师思路分析：
    ...
    public boolean add(House newHouse) {
        ...
        //我们程序员需要设计一个id自增长机制, 然后更新newHouse的id
