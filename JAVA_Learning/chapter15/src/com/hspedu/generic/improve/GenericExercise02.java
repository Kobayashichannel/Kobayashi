    ...
    public static void main(String[] args) {
        ...
        employees.sort(new Comparator<Employee>() {
            ...
            public int compare(Employee emp1, Employee emp2) {
                //先按照name排序,如果name相同,则按生日日期先后排序。[即：定制排序]
                //先对传入的参数进行验证
 ...
 * (4) MyDate类包含: private成员变量 month,day,year; 并为每一个属性定义 getter,setter 方法
 * (5) 创建该类的 3个 对象,并把这些对象放入 ArrayList 集合中(ArrayList 需要使用泛型来定义), 对集中的元素进行排序
 *
 * 排序方式: 调用ArrayList 的 sort 方法, 传入 Comparator对象[使用泛型],先按
 *
 */
