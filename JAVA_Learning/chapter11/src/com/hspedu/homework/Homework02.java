...
4.在Frock类中声明serialNumber(序列号)属性,并提供对应get方法
5.在Frock类的构造器中,通过调用getNextNum方法为Frock对象获取唯一序列号,赋给serialNumber属性。
6.在TestFrock类的main方法中,分别创建三个Frock 对象,并打印三个对象的序列号,验证是否按100递增
 */
class Frock {
...
class TestFrock {
    public static void main(String[] args) {
        ...
        Frock frock = new Frock(); //序列号 100300
        Frock frock1 = new Frock(); //序列号 100400
        Frock frock2 = new Frock(); //序列号 100500
