...
FROM employees, department; #查询出2889条记录

#错误的方式
SELECT employee_id,department_name
FROM employees CROSS JOIN department; #查询出2889条记录
...
#4. 如果查询语句中出现了多个表都存在的字段,则必须指明此字段存在的表。
...
# 右外连接: 两个表在连接过程中除了返回满足连接条件的行以外还返回左表中不满足条件的行,这种连接称为右外连接
...
SELECT last_name, departmet_name
...
# NATURAL JOIN: 它会帮你自动查询两张连接表中`所有的相同的字段`, 然后进行`等值连接`。
SELECT employee_id,last_name,department_name
