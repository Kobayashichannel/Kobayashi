...
SELECT AVG(salary),SUM(salary),AVG(salary)
FROM employees;
#如下的操作没有意义
...
# 如何需求统计表中的记录数,使用COUNT(*)、COUNT(1)、COUNT(具体字段) 哪个效率更高呢?
...
#要求1: 如果过滤条件中使用了聚合函数,则必须使用HAVING来替代WHERE。否则,报错。
...
#练习: 查询各个部门id为10,20,30,40这4个部门中最高工资比10000高的部门信息
#方式1: 推荐,执行效率高于方式2.
SELECT department_id,MAX(salary)
FROM employees
