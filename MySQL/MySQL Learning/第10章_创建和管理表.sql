...
#8. 对比 TRUNCATE TABLE 和 DELECT FROM
# 相同点: 都可以实现对表中所有数据的删除, 同时保留表结构
# 不通电:
#	  TRUNCATE TABLE: 一旦执行此操作,表数据可以全部清除。同时,数据是不可以回滚的。
#	  DELETE FROM: 一旦执行此操作,表数据可以全部清除(不带WHERE)。同时,数据是可以回滚的

/*
9. DDL 和 DML 的说明
  ① DDL的操作一旦执行,就不可回滚。 指令SET autocommit = FALSE对DDL操作失效.(因为在执行完
     DDL操作之后，一定会执行一次COMMIT.而此COMMIT操作不受SET autocommit = FALSE影响的.)
  
  ② DML的操作默认情况,一旦执行,也是不可以回滚的.但是,如果执行DML之前,执行了
