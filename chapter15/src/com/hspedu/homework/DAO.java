...
/**
 *
 *  * 定义个泛型 DAO<T>,在其中定义一个Map 成员变量 ,Map 的键为 String 类型, 值为 T 类型.
 *  * 分别创建以下方法:
 *  * (1) public void save(String id,T entity): 保存 T 类型的对象到 Map 成员变量中
 *  * (2) public T get(String id): 从 map 中获取 id 对应的对象
 *  * (3) public void update(String id, T entity): 替换 map 中 key 为 id的内容,改为 entity
...
public class DAO<T> { //泛型类
    ...
    public List<T> list() {
        ...
        for (String key : keySet) {
            //map.get(key) 返回就是 User对象 -> ArrayList b
    ...
    public void save(String id, T entity) { //把entry保存到map
        map.put(id, entity);
    }
}
